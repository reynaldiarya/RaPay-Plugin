...
class WC_Gateway_BTN extends WC_Payment_Gateway
{
    ...
    public function __construct()
    {
        ...
        $this->icon               = apply_filters('woocommerce_bank_btn_icon', $icon_url);
        $this->has_fields         = false;
        $this->method_title       = __('Bank BTN', 'beipay-bank-e-money-indonesia');
        $this->method_description = __('Lakukan pembayaran melalui transfer langsung ke rekening Bank BTN.', 'beipay-bank-e-money-indonesia');
    ...
    public function init_form_fields()
    {
        $this->form_fields = array(
            'enabled'         => array(
                ...
                'type'    => 'checkbox',
                'label'   => __('Enable Bank BTN', 'beipay-bank-e-money-indonesia'),
                'default' => 'no',
            ),
            'title'           => array(
                ...
                'default'     => __('Transfer Bank BTN', 'beipay-bank-e-money-indonesia'),
                'desc_tip'    => true,
            ),
            'enable_icon' => array(
            ...
            'description'     => array(
                ...
                'default'     => __('Lakukan pembayaran langsung ke rekening Bank BTN kami. Mohon gunakan ID Pesanan Anda sebagai referensi pembayaran. Pesanan Anda tidak akan dikirimkan hingga dana telah masuk ke rekening kami.', 'beipay-bank-e-money-indonesia'),
                'desc_tip'    => true,
            ),
            'instructions'    => array(
    ...
    public function generate_account_details_html()
    {
        ...
        ?>
                ...
                <script type="text/javascript">
                    jQuery(function() {
                        jQuery('#bank_btn_accounts').on('click', 'a.add', function() {

                            var size = jQuery('#bank_btn_accounts').find('tbody .account').length;

                            jQuery('<tr class="account">\
									<td class="sort"></td>\
									<td><input type="text" name="bank_btn_account_name[' + size + ']" /></td>\
									<td><input type="text" name="bank_btn_account_number[' + size + ']" /></td>\
									<td><input type="text" name="bank_btn_sort_code[' + size + ']" /></td>\
									<td><input type="text" name="bank_btn_iban[' + size + ']" /></td>\
									<td><input type="text" name="bank_btn_bic[' + size + ']" /></td>\
								</tr>').appendTo('#bank_btn_accounts table tbody');
...
<?php
    ...
    public function save_account_details()
    {
        ...
        do_action('woocommerce_update_option', array('id' => 'woocommerce_bank_btn_accounts'));
    ...
    public function email_instructions($order, $sent_to_admin, $plain_text = false)
    {
        if (! $sent_to_admin && self::ID === $order->get_payment_method()) {
             ...
             */
            $instructions_order_status = apply_filters('woocommerce_bank_btn_email_instructions_order_status', OrderStatus::ON_HOLD, $order);
    ...
    private function bank_details($order_id = '')
    {
        ...
        $bank_btn_accounts = apply_filters('woocommerce_bank_btn_accounts', $this->account_details, $order_id);
        ...
        if (! empty($bank_btn_accounts)) {
            ...
            foreach ($bank_btn_accounts as $bank_btn_account) {
                ...
                $account_fields = apply_filters(
                    'woocommerce_bank_btn_account_fields',
                    array(
            ...
            if ($has_details) {
                echo '<section class="woocommerce-bank-btn-bank-details"><h2 class="wc-bank-btn-bank-details-heading">' . esc_html__('Rincian rekening bank kami', 'beipay-bank-e-money-indonesia') . '</h2>' . wp_kses_post(PHP_EOL . $account_html) . '</section>';
            }
        }
    }
    ...
    public function process_payment($order_id)
    {
        ...
        if ($order->get_total() > 0) {
             ...
             * @param object $order          The order object.
             */
            $process_payment_status = apply_filters('woocommerce_bank_btn_process_payment_order_status', OrderStatus::ON_HOLD, $order);
            // Mark as on-hold (we're awaiting the payment).
            $order->update_status($process_payment_status, __('Menunggu pembayaran dari Bank BTN.', 'beipay-bank-e-money-indonesia'));
        } else {
