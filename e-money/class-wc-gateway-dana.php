...
class WC_Gateway_Dana extends WC_Payment_Gateway
{
    ...
    public function __construct()
    {
        ...
        $this->method_title       = __('Dana', 'beipay-bank-e-money-indonesia');
        $this->method_description = __('Lakukan pembayaran melalui transfer langsung ke rekening Dana.', 'beipay-bank-e-money-indonesia');
    ...
    public function init_form_fields()
    {
        $this->form_fields = array(
            'enabled'         => array(
                ...
                'type'    => 'checkbox',
                'label'   => __('Enable Dana', 'beipay-bank-e-money-indonesia'),
                'default' => 'no',
            ),
            'title'           => array(
                ...
                'default'     => __('Transfer Dana', 'beipay-bank-e-money-indonesia'),
                'desc_tip'    => true,
            ),
            'enable_icon' => array(
            ...
            'description'     => array(
                ...
                'default'     => __('Lakukan pembayaran langsung ke rekening Dana kami. Mohon gunakan ID Pesanan Anda sebagai referensi pembayaran. Pesanan Anda tidak akan dikirimkan hingga dana telah masuk ke rekening kami.', 'beipay-bank-e-money-indonesia'),
                'desc_tip'    => true,
            ),
            'instructions'    => array(
...
<?php
    ...
    private function bank_details($order_id = '')
    {
        ...
        if (! empty($dana_accounts)) {
            ...
            if ($has_details) {
                echo '<section class="woocommerce-dana-bank-details"><h2 class="wc-dana-bank-details-heading">' . esc_html__('Rincian rekening bank kami', 'beipay-bank-e-money-indonesia') . '</h2>' . wp_kses_post(PHP_EOL . $account_html) . '</section>';
            }
        }
    }
    ...
    public function process_payment($order_id)
    {
        ...
        if ($order->get_total() > 0) {
            ...
            $order->update_status($process_payment_status, __('Menunggu pembayaran dari Dana.', 'beipay-bank-e-money-indonesia'));
        } else {
